---
title: "Selected StatLab Consult Code Excerpts"
...
Below are excerpts from code I prepared for a few of the statistical and programming consultations I've received as part of my work for the UVa Library's StatLab. I've removed identifying information from code chunks and comments. Most consultations involve a meeting at which I learn more about the inquirer's question or problem, discuss possible solutions, and walk through any code I've drafted with them.

---

<details><summary>Calculating the entropy of images</summary><br/>

I received a consult request from someone interested in quantifying and comparing the amount of disorder in images of greenspaces. I proposed one approach: Calculating the information entropy of pixel intensities in each image. I prepared a function that read in images, extracts pixel-wise grayscale values, and calculates information entropy from a binned version of those values.
...
I received a consult request from someone looking to analyze the effect of an education intervention. Each participant in the data had two to three associated measurements, with a (categorical) intervention value and an outcome measure (an exam score) at each time point. I generally incline toward using mixed-effects models to handle data of this sort, but in this case, to accommodate the inquirer's particular level of comfort with some statistical methods and not others, I proposed accounting for the dependent structure of the data using clustered standard errors. I worked up this example:
...
I received an inquiry from someone looking to calculate Bray-Curtis dissimilarity indexes in R. I simulated some relevant data and worked up the following example:
...
I received a consult request from someone with two large lists of data frames, where each list element contained some number of years worth of data on a particular measure (one list housed data frames containing yearly values on one measure, X; the other list housed data frames containing values on a second measure, Y). The inquirer was looking to identify which matched pairs of list elements contained mismatched years of data (e.g., the i^th^ element of List 1 contained X values for 2000:2015, but the i^th^ element of List 2 only contained Y values 2000:2008 and 2011:2015). When mismatched were identified for a given matched pair of list elements, the inquirer was looking to drop those years from the elements. I proposed the following:
...
I received an inquiry from someone interested in calculating transition matrices for a number of observations that were each measured at five time points on a scale from 0:3. The inquirer had a function in hand for calculating transition matrices---`Markovmatrix2` below---but the function wasn't acting as desired: It was omitting rows and columns of the matrices that were needed for the inquirer's intended analysis. I proposed the following solution:
...
#   E.g., if an observation only has 0/2 scores (i.e., no score of 1 or 3 at any time point), the output is a 2x2 matrix with 0/2 along the rows and 0/2 along the columns, like:
#       0       2
#   0   0.00    0.33
#   2   0.33    0.33
# Each matrix, however, needs to be 4x4, with 0:3 along the rows and the columns.
# I wrote the function below that checks which rows and columns, from 0:3, are missing for each observation and adds those in (if there are any)
